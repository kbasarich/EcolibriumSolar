...
Public Function SendDataToFathymFlow() As String
    ...
    If (SugarAccountId = vbNullString Or SugarAccountId = "0") Then
        account = CheckDuplicateAccount
    End If
    
    ApplicationProvider.UpdateProgress
    SugarAccountId = CreateOrUpdateAccount(account)
...
Public Function CreateOrUpdateAccount(ByVal accountName As String) As String
    
    ApplicationProvider.SetStatus ("Creating/Updating Account: " & accountName)

    Dim request As New Dictionary
    
    request("ActionType") = "CreateOrUpdate"
    request("ModuleType") = "Accounts"
    request("Object") = ExcelProvider.GetAccount(accountName, SugarAccountId)
    ...
    ApplicationProvider.SetStatus ("Done Creating/Updating Account: " & accountName & " : " & responseObject("id"))
...
Public Function CreateOrUpdateQuote() As String
    ...
    request("Object") = ExcelProvider.GetQuote(SugarQuoteId)
...
Public Function UpdateQuote(ByVal quoteId As String, ByVal products As Collection) As Boolean
    ...
    If (products.count > 0) Then
        ...
        request("ActionType") = "CreateOrUpdate"
        request("ModuleType") = "Quotes"
        request("Object") = ExcelProvider.GetQuote(quoteId, products)
